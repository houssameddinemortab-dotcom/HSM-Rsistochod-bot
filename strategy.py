...
def is_price_at_order_block(
    ...
    Validation directionnelle STRICTE :
      Bearish OB (SELL) : le prix doit être <= ob_high + tolérance minimale
                          Si prix > ob_high → prix a cassé l'OB vers le haut
                          → ce serait un BUY, pas un SELL → signal invalide

      Bullish OB (BUY)  : le prix doit être >= ob_low - tolérance minimale
                          Si prix < ob_low → prix a cassé l'OB vers le bas
                          → signal invalide
    """
    for ob in reversed(order_blocks):
        ...
        # Vérification de base : prix dans la zone élargie
        if not (low_z <= price <= hi_z):
            continue

        # Validation directionnelle stricte
        if ob_type == "BEARISH" and price > ob["high"]:
            # Prix au-dessus du haut de l'OB Bearish = cassure haussière
            # Ce n'est PAS un signal SELL valide
            logger.debug(
                "OB BEARISH rejeté : prix=%.2f > ob_high=%.2f → cassure haussière, signal invalide",
                price, ob["high"]
            )
            continue

        if ob_type == "BULLISH" and price < ob["low"]:
            # Prix en-dessous du bas de l'OB Bullish = cassure baissière
            logger.debug(
                "OB BULLISH rejeté : prix=%.2f < ob_low=%.2f → cassure baissière, signal invalide",
                price, ob["low"]
            )
            continue
        ...
        ob["retested"] = True
        logger.debug(
            "OB %s retest valide : prix=%.2f dans [%.2f – %.2f]",
            ob_type, price, low_z, hi_z
        )
        return ob
...
def calculate_entry(signal: Signal, price: float, ob: dict) -> dict:
    ...
    BUY  : SL = sous l'OB (toujours < entry) | TP = entry + distance*ratio
    SELL : SL = au-dessus de l'entrée (forcé) | TP = entry - distance*ratio

    Fix : pour SELL, le SL est calculé comme entry + SL_PIPS*0.01
    pour garantir qu'il est TOUJOURS au-dessus de l'entrée,
    indépendamment de la position de l'OB.
    """
    capital  = capital_tracker.get_capital()
    risk_usd = round(capital * RISK_PERCENT / 100, 2)

    if signal == "BUY":
        # SL sous le bas de l'OB
        sl_ref   = min(ob["low"], price)          # plancher = bas de l'OB ou prix si déjà dessous
        sl       = round(sl_ref - SL_PIPS * 0.01, 2)
        entry    = price
        tp       = round(entry + abs(entry - sl) * TP_RATIO, 2)
    else:
        # SL au-dessus du haut de l'OB — toujours supérieur à l'entrée
        sl_ref   = max(ob["high"], price)          # plafond = haut de l'OB ou prix si déjà dessus
        sl       = round(sl_ref + SL_PIPS * 0.01, 2)
        entry    = price
        tp       = round(entry - abs(sl - entry) * TP_RATIO, 2)

    # Vérifications de cohérence (garde-fous)
    if signal == "BUY"  and sl >= entry:
        sl = round(entry - SL_PIPS * 0.01, 2)
        tp = round(entry + abs(entry - sl) * TP_RATIO, 2)
