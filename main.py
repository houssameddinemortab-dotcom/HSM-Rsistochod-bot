...
Toutes les 300 secondes :
  - Si trade actif : vérifie TP/SL → log_result()
  - Sinon : analyse + message veille (+ log_entry() si signal)

Modification vs version précédente :
  [+] import trade_logger
  [+] trade_logger.log_entry(analysis) après open_trade()
  [+] trade_logger.log_result() remplace notify_tp/notify_sl directs
"""
...
def open_trade(analysis: dict) -> None:
    """Enregistre le trade actif et envoie le message d'entrée complet."""
    global active_trade
    ...
    trade_logger.log_entry(analysis)   # ← NOUVEAU : message d'entrée complet


def check_active_trade(candles: list[dict]) -> bool:
    """Vérifie si TP ou SL est atteint sur la dernière bougie."""
    global active_trade
    if not active_trade:
        return False
    ...
    if tp_hit:
        logger.info("🎯 TP | %s | +%.2f USD", signal, active_trade["profit"])
        trade_logger.log_result("TP", active_trade, last["datetime"])  # ← NOUVEAU
        active_trade = None
        return True

    if sl_hit:
        logger.info("🛑 SL | %s | -%.2f USD", signal, active_trade["risk"])
        trade_logger.log_result("SL", active_trade, last["datetime"])  # ← NOUVEAU
        active_trade = None
        return True
...
def run_cycle() -> None:
    ...
    telegram_bot.notify_cycle(analysis)   # message de veille (toujours)

    if analysis.get("signal") in ("BUY", "SELL"):
        open_trade(analysis)   # log_entry() appelé ici
